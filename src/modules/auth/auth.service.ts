...
export const authService = {
  async register(input: RegisterInput) {
    ...
    // Create the user + profile atomically. The account is active immediately
    // and can log in without any email verification step.
  ...
  async forgotPassword(input: ForgotPasswordInput) {
    ...
    // TODO: Wire up an email provider to deliver the reset link. The email
    // verification system was removed, so no SMTP transport is configured.
    // For now the token is surfaced in non-production for end-to-end testing.
    return {
